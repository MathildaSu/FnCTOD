...
from src.multiwoz.utils.clean_dataset import clean_slot_values
from src.multiwoz.utils.reader import MultiWozReader
from src.multiwoz.utils.config import Config20, Config21, Config23
from src.multiwoz.utils import ontology
...
class BLEUScorer(object):
    ...
    def score(self, parallel_corpus):
        ...
        bp = 1 if c > r else math.exp(1 - self.safe_division(float(r), float(c)))
...
class MultiWozEvaluator(object):
    def __init__(self, dataset_version="2.0", tokenizer_path="t5-base"):
        ...
        self.cfg = Config(data_prefix="/Users/tsu/Documents/code/FnCTOD/data/multiwoz/data/")
    ...
    def wrap_evaluation_result(self, result_list):
        ...
        for one_id in id_list:
            ...
            entry = {"dial_id": one_id, "turn_num": session_len}
            for f in field[2:]:
                entry[f] = ""  # ???
    ...
    def validation_metric(self, data):
        ...
        (
            ...
            total_successes,
            total_matches,
        ) = self.context_to_response_eval(
            data, same_eval_as_cambridge=self.cfg.same_eval_as_cambridge
        )
        return bleu, success, match, total_successes, total_matches, dial_num
    ...
    def _get_metric_results(self, data, domain="all", file_list=None):
        ...
        (
            ...
            data, file_list, same_eval_as_cambridge=self.cfg.same_eval_as_cambridge
        )
        req_slots_acc = {}
        ...
        for dial_id in dials:
            ...
            for turn_num, turn in enumerate(dial):
                ...
                if self.cfg.enable_dspn:
                    ...
                    turn_dom_bs = []
                    if (
                        self.cfg.enable_bspn
                        and not self.cfg.use_true_bspn_for_ctr_eval
                        and (self.cfg.bspn_mode == "bspn" or self.cfg.enable_dst)
                    ):
                        constraint_dict = self.reader.bspan_to_constraint_dict(
                            turn["bspn_gen"]
                        )
    ...
    def context_to_response_eval(
        ...
        return succ_rate, match_rate, counts, dial_num, successes, matches
            ...
            for domain in goal.keys():
                ...
                if "[value_name]" in sent_t or "[value_id]" in sent_t:
                    if domain in ["restaurant", "hotel", "attraction", "train"]:
                        ...
                        if (
                            not self.cfg.use_true_curr_bspn
                            and not self.cfg.use_true_bspn_for_ctr_eval
                        ):
                            bspn = turn["bspn_gen"]
                        else:
